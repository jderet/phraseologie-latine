...
- Trois composants JavaScript seulement : l'éditeur de traduction (`static/js/editor.js`), la lecture annotée (`static/js/reading.js`, étape 6, choix du 14 septembre 2026) et le dessin du schéma (`static/js/schema.js`, étape 9, choix du 15 septembre 2026). Sans eux, les pages fonctionnent par formulaires ; la lecture montre ses soulignements et des liens vers les fiches ; le schéma s'écrit à la main. Le PDF est enregistré par le navigateur depuis la page imprimable. S'y ajoute un module de confort, `static/js/macrons.js`, chargé sur tout le site : dans un champ marqué `lang="la"`, deux voyelles identiques de suite donnent une voyelle longue (`aa` donne `ā`, la retaper redonne `aa` ; avec `i`, `ii` donne `ī`, `iii` donne `iī`, `iiii` redonne `iii`). Ce n'est pas un quatrième composant : il ne réagit qu'à la frappe, laisse le texte collé intact, et sans lui les champs s'écrivent comme avant (raccourci `a=` de l'éditeur, barre des voyelles longues). Tout champ où l'on écrit du latin porte donc `lang="la"`.
...
| genre, thème d'un texte à traduire | `SourceText.genres`, `SourceText.themes` (`translations/classification.py`) |
