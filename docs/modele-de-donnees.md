...
- **Texte source** (`SourceText`) : titre, auteur, langue, adresse d'origine, licence déclarée, statut juridique (domaine public, licence libre), ajouté par, texte découpé, état (numéro du dernier changement de ses phrases, 0 à l'ajout). Un retour arrière ne rétablit ni l'état ni le texte découpé.
- **Segment** (`Segment`) : texte source, position (tri de toutes les phrases, retirées comprises), numéro dans le texte actuel (vide pour une phrase retirée), phrase, début de paragraphe, changement qui l'a ajoutée (0 : le texte d'origine), changement qui l'a retirée. Une phrase ne se modifie jamais : un changement retire des phrases et en ajoute de nouvelles ; les anciennes restent pour les étapes qui les ont figées.
...
- **Étape** (`VersionStep`) : version, numéro (1, 2, 3… dans la version), message, auteur, date, créée pendant le brouillon ou non, état du texte source (l'étape montre les phrases source de cet état). Seul l'auteur de la version la crée, quand le texte de travail ou le texte source a changé depuis l'étape précédente, ou qu'une justification attend de paraître ; publier en crée toujours une. Une étape ne se modifie pas et sa création ne compte pas dans la limite des nouveaux comptes. Elle est visible de tous quand la version est publiée, sauf une étape du brouillon si l'auteur a choisi de ne pas les montrer. Masquée après un signalement, elle sort de l'historique public ; le public voit alors la dernière étape non masquée.
