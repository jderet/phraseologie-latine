...
class SurveyServiceTests(SurveyTestCase):
    def test_the_core_occurrences_become_automatic_attestations(self):
        ...
        self.assertEqual((survey.found, survey.added, survey.remaining), (3, 2, 0))
        ...
        self.assertEqual(
            ...
            [list(self.more_words[:2]), list(self.good_words[1:])],
        )
        ...
        self.assertEqual(Revision.objects.filter(comment="Relevé automatique").count(), 2)
    ...
    def test_a_new_survey_leaves_known_and_rejected_occurrences_aside(self):
        ...
        survey = survey_unit(self.unit, self.other)
        self.assertEqual((survey.found, survey.added), (3, 0))
        self.assertEqual(self.unit.attestations.count(), 3)
    ...
    def test_an_attestation_covering_more_words_counts_as_known(self):
        ...
        self.assertEqual(survey_unit(self.unit, self.other).added, 2)
    ...
    def test_a_survey_adds_a_limited_number_and_the_next_goes_on(self):
        with mock.patch("phraseology.services.SURVEY_LIMIT", 1):
            ...
            survey = survey_unit(self.unit, self.other)
            self.assertEqual((survey.added, survey.remaining), (1, 0))
...
class BatchReviewTests(SurveyTestCase):
    def setUp(self):
        super().setUp()
        survey_unit(self.unit, self.other)
        self.ids = list(self.automatic().values_list("pk", flat=True))
...
class SurveyPagesTests(SurveyTestCase):
    ...
    def test_editors_launch_the_survey(self):
        ...
        self.assertContains(response, "2 attestations ajoutées au relevé.")
        self.assertContains(response, "3 occurrences repérées automatiquement le")
