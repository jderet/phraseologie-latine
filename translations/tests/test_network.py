...
class NetworkTests(TranslationTestCase):
    def copy(self, version, user):
        return copy_version(public_step(version), TranslationVersion(style=version.style), user)

    def test_tree_of_copies_with_their_drift(self):
        ...
        second_copy = self.copy(first_copy, self.reviewer)
        ancestors, nodes = copy_network(self.reviewer, original)
        self.assertEqual(ancestors, [])
        self.assertEqual(
            [(node.version, node.depth) for node in nodes], [(first_copy, 1), (second_copy, 2)]
        )
        self.assertEqual(nodes[0].differing, 1)
        # A draft copy is seen by its writers only.
        ...
        self.assertEqual(ancestors, [original, first_copy])

    def test_page(self):
        original = make_published_version(self.author, self.project)
        response = self.client.get(reverse("translations:version_network", args=[original.pk]))
        self.assertContains(response, "Personne n’a encore copié cette version.")
